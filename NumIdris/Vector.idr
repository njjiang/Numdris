...
argmax v = let m = max v in fromMaybe (FZ) (elemIndex m v)

||| find the index of the min element in some non-empty vector
||| @ v the vector
argmin : (Ord t, Field t) => (v : Vect (S n) t) -> Fin (S n)
argmin v = let m = min v in fromMaybe (FZ) (elemIndex m v)
...
testv1 : Vect 3 Double
testv1 = fromList [1,3,2]

testv2 : Vect 2 Double
testv2 = fromList [1,2]
