-- copied from
-- https://github.com/idris-lang/Idris-dev/blob/8897f649b6aadd00f01651ac4e2dc08b6bbb7c6d/libs/contrib/Data/SortedMap.idr
...
export
lookupDefault : k -> v -> SortedMap k v -> v
lookupDefault k const map with (lookup k map)
    | Just val = val
    | Nothing = const
...
export
update : k -> (v -> v) -> SortedMap k v -> SortedMap k v
update key f map with (lookup key map)
    | Just val = insert key (f val) (delete key map)
    | Nothing = map
